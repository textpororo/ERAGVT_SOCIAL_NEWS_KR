...
IF CFLAG:42 == 400
	...
	IF RAND:4 == 0
		...
		PRINTFORMW せめて声だけはと必死に堪えながら、%CALLNAME%은(는) 肩を震わせて絶頂に達した。
	ELSEIF RAND:3 == 0
		PRINTFORML 「ひうぁぁあっ！？」
	...
	ELSEIF RAND:2 == 0
		...
		PRINTFORMW 休息を得るはずのベッドの中で、また一度%CALLNAME%은(는) 望まぬ絶頂に導かれた。
	ELSE
		PRINTFORML (なんで、%SELF_CALL()%、こんな……）
		PRINTFORML 明るく仲間たちが語り合う傍で、静かに%CALLNAME%은(는) 絶頂をこらえていた。
...
SELECTCASE RAND:2
	CASE 0
		PRINTFORML  今や%CALLNAME%은(는) 貴重な資料の宝庫でしかなく、衣服はおろか布一枚まとうことすら許されず
	...
	CASEELSE
		...
		PRINTFORML  촉수に孕まされ戦えなくなった%CALLNAME%은(는) 分娩台に拘束され、貴重なデータを提供させられ続けていた。
